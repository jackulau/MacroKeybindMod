...
> **Most of our iterators are single-var; `players`, `effects`, `properties`, `enchantments`, and `running` are multi-var.** For the single-var iterators `foreach(&v, <iterator>)` binds **one** loop variable to **one** value per element — the primary name: `teams` → team name, `objectives` → objective name (`hotbar`/`inventory` → the slot's item id). The multi-variable Klacaiba columns below (`PLAYERUUID`, `TEAMPREFIX`, `SCOREVALUE`, …) are **not exposed** for those iterators. The exceptions are **`players`**, **`effects`**, **`properties`**, **`enchantments`**, and **`running`** (MULTI-VAR iterators): each binds the loop var to a primary AND exposes fixed-name vars per element (read in-body as `%PLAYERNAME%` / `%EFFECTNAME%` / `%PROPNAME%` / `%ENCHANTMENT%` / `%MACRONAME%` etc.), mirroring MKB's `ScriptedIteratorPlayers`/`ScriptedIteratorEffects`/`ScriptedIteratorProperties`/`ScriptedIteratorEnchantments`/`ScriptedIteratorRunning` — see the tables below. `players` iterates the connection's full **online roster** (`getOnlinePlayers()` — the player-info/tab list, every connected player, matching MKB's `.d()`, **not** just the nearby loaded entities) and exposes `%PLAYERNAME%` = the GameProfile name; the other five `PLAYER*` rows (`PLAYERUUID`/`PLAYERPING`/…) are Klacaiba-documented *beyond* MKB itself and remain unexposed.
