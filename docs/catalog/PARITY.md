...
| **Iterators** | 8 | **11**: 7 shared (`env` `running` `array` `players` `effects` `properties` `enchantments`) + 4 ours-only (`hotbar` `inventory` `teams` `objectives`) | mostly single-var `foreach`; `players` + `effects` + `properties` + `enchantments` + `running` are MULTI-VAR (players PLAYERNAME per online-roster player from `getOnlinePlayers()`; effects EFFECTID/EFFECT/EFFECTNAME/EFFECTPOWER/EFFECTTIME per active potion effect; properties PROPNAME/PROPVALUE per looking-at block-state property; enchantments ENCHANTMENT/ENCHANTMENTNAME/ENCHANTMENTPOWER per held-item enchantment; running MACROID/MACRONAME/MACROTIME per executing wait-suspended macro), mirroring MKB `ScriptedIteratorPlayers`/`ScriptedIteratorEffects`/`ScriptedIteratorProperties`/`ScriptedIteratorEnchantments`/`ScriptedIteratorRunning`; only MKB-only `controls` (GUI-overlay widgets, no analogue) not reimplemented. **`env` deliberately diverges** (tested intent, not a gap): it iterates our currently-set scalar var NAMES (sigil-keyed, case-inclusive — pinned by `IteratorTest`'s "foreach env iterates the set scalar variables") rather than MKB's `ScriptedIteratorEnvironment`, which unions built-in env names + user vars filtered to `^~?[A-Z_]+[0-9]*$`; our built-in env vars are host getters referenced directly (`%XPOS%`), not an enumerable set, so iterating them is an intentional non-goal |
...
!!! note "Engine-complete; Fabric realization, in layers"
    ...
    the chat-filter pipeline, 25 events, 11 iterators (incl. the multi-var `effects`/`properties`/`enchantments`/`running`), and ~108 player / world / settings / armor /
