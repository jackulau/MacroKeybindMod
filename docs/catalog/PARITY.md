...
| **Iterators** | 8 | **9**: 5 shared (`env` `running` `array` `players` `effects`) + 4 ours-only (`hotbar` `inventory` `teams` `objectives`) | mostly single-var `foreach`; `effects` is our first MULTI-VAR iterator (EFFECTID/EFFECT/EFFECTNAME/EFFECTPOWER/EFFECTTIME per element, mirrors MKB `ScriptedIteratorEffects`); MKB-only `enchantments`/`properties` (multi-var, same engine mechanism, enqueued) + `controls` (MKB GUI-overlay widgets, no analogue) not reimplemented; `running` = wait-suspended macro names |
...
!!! note "Engine-complete; Fabric realization, in layers"
    ...
    the chat-filter pipeline, 25 events, 9 iterators (incl. the multi-var `effects`), and ~108 player / world / settings / armor /
...
All 127 keywords + 16 sigils + our 9 iterators (5 shared with MKB including the multi-var `effects` + 4 ours-only; see the Iterators row) are implemented; the async runner, world reads, settings,
