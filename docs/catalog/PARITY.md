...
| **Iterators** | 8 | **11**: 7 shared (`env` `running` `array` `players` `effects` `properties` `enchantments`) + 4 ours-only (`hotbar` `inventory` `teams` `objectives`) | mostly single-var `foreach`; `effects` + `properties` + `enchantments` + `running` are MULTI-VAR (effects EFFECTID/EFFECT/EFFECTNAME/EFFECTPOWER/EFFECTTIME per active potion effect; properties PROPNAME/PROPVALUE per looking-at block-state property; enchantments ENCHANTMENT/ENCHANTMENTNAME/ENCHANTMENTPOWER per held-item enchantment; running MACROID/MACRONAME/MACROTIME per executing wait-suspended macro), mirroring MKB `ScriptedIteratorEffects`/`ScriptedIteratorProperties`/`ScriptedIteratorEnchantments`/`ScriptedIteratorRunning`; only MKB-only `controls` (GUI-overlay widgets, no analogue) not reimplemented |
