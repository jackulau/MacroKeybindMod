...
| **Iterators** | 8 | **10**: 6 shared (`env` `running` `array` `players` `effects` `properties`) + 4 ours-only (`hotbar` `inventory` `teams` `objectives`) | mostly single-var `foreach`; `effects` + `properties` are MULTI-VAR (effects EFFECTID/EFFECT/EFFECTNAME/EFFECTPOWER/EFFECTTIME per active potion effect; properties PROPNAME/PROPVALUE per looking-at block-state property), mirroring MKB `ScriptedIteratorEffects`/`ScriptedIteratorProperties`; MKB-only `enchantments` (multi-var, same engine mechanism, enqueued) + `controls` (MKB GUI-overlay widgets, no analogue) not reimplemented; `running` = wait-suspended macro names |
...
!!! note "Engine-complete; Fabric realization, in layers"
    ...
    the chat-filter pipeline, 25 events, 10 iterators (incl. the multi-var `effects`/`properties`), and ~108 player / world / settings / armor /
...
All 127 keywords + 16 sigils + our 10 iterators (6 shared with MKB including the multi-var `effects`/`properties` + 4 ours-only; see the Iterators row) are implemented; the async runner, world reads, settings,
